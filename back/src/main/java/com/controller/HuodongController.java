                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            HuodongEntity huodongEntity = new HuodongEntity();
//                            huodongEntity.setHuodongUuidNumber(data.get(0));                    //报考活动编号 要改的
//                            huodongEntity.setHuodongName(data.get(0));                    //报考活动名称 要改的
//                            huodongEntity.setHuodongPhoto("");//详情和图片
//                            huodongEntity.setHuodongAddress(data.get(0));                    //活动地点 要改的
//                            huodongEntity.setJiezhiTime(sdf.parse(data.get(0)));          //截至时间 要改的
//                            huodongEntity.setZanNumber(Integer.valueOf(data.get(0)));   //赞 要改的
//                            huodongEntity.setCaiNumber(Integer.valueOf(data.get(0)));   //踩 要改的
//                            huodongEntity.setHuodongTypes(Integer.valueOf(data.get(0)));   //报考活动类型 要改的
//                            huodongEntity.setHuodongClicknum(Integer.valueOf(data.get(0)));   //报考活动热度 要改的
...
//                            huodongEntity.setCreateTime(date);//时间
                        ...
                        if(huodongEntities_huodongUuidNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [报考活动编号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
